...
class MainActivity : AppCompatActivity(), MainContract.View{
    ...
    //2. MVP [ Model - View - Presenter ] -  view와 model 완전 분리 특징이 가장 두드러짐 [ 뷰와 Presenter 가 해야할 작업을 미리 인터페이스로 규격화 한 것이 특징. 모듈화된 작업 탬플릿을 만들때 용이 한 구조임]
    //  1) Model : MVC패턴의 모델과 같은 역할 [ 데이터 취급 : Item, Person, ItemModel ... ]
    //  2) View  : 사용자가 볼 화면 및 이벤트 처리 [ activity_main.xml, MainActivity.kt, fragment_my, MyFragment.kt ..]
...
// 4. view 안에서 model을 참조하고 있지 않기에 model 의 변화에 영향받지 않음.

// ## MVP 단점 ##
// 1. MVC보다 만들어야 할 기본 파일들이 많아서 구조가 더 복잡해 보임
// 2. view와 presenter 가 1:1로 대응되어 파일들이 만들어지기에 파일이 엄청 많아짐.
